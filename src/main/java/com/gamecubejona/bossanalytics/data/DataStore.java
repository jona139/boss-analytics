...
/**
 * SQLite-backed local data store for all kill/raid records.
 * 
 * Design: This is an abstraction layer. When you add a backend later,
 * create a RemoteDataStore implementing the same interface and swap via config.
 */
@Slf4j
@Singleton
public class DataStore
{
    ...
    private void initSchema() throws SQLException
    {
        try (Statement stmt = connection.createStatement())
        {
            ...
            stmt.execute(
                ...
                + "  boosted_levels TEXT,"         // JSON
                + "  combat_achievement_tier INTEGER DEFAULT 0,"
                + "  equipped_item_ids TEXT,"      // JSON
                + "  equipped_item_names TEXT,"    // JSON
                + "  inventory_item_ids TEXT,"     // JSON
                + "  kill_count INTEGER,"
            ...
            // Raid-specific records (linked to kills)
            stmt.execute(
                ...
                + "  rooms TEXT,"                  // JSON array of room records
                + "  route TEXT,"
            ...
            stmt.execute("INSERT OR IGNORE INTO schema_version VALUES (" + SCHEMA_VERSION + ")");
        }
    }
    ...
    public List<KillRecord> getKillsByBoss(String bossName) throws SQLException
    {
        return getKillsByBoss(bossName, 0);
    }
